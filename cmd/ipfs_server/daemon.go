...
func daemonFunc(req cmds.Request, res cmds.Response) {
	...
	if val, err := repo.GetConfigKey("Casper.TelegramAddress"); err != nil || val.(string) == config.DefaultCasperTelegramAddress {
		fmt.Printf("Your current telegram address is not set. Please type it and press enter\n>")
		...
		err = repo.SetConfigKey("Casper.TelegramAddress", str)
		if err != nil {
			res.SetError(err, cmds.ErrNormal)
			return
		}
	...
	if cfg.Casper.TelegramAddress == config.DefaultCasperTelegramAddress {
		fmt.Printf("Your current telegram address is not set. Please type it and press enter\n> ")
		scan := bufio.NewScanner(os.Stdin)
		if scan.Scan() {
			cfg.Casper.TelegramAddress = scan.Text()
			err = repo.SetConfig(cfg)
			if err != nil {
				res.SetError(err, cmds.ErrNormal)
				return
			}
			log.Debugf("new telegram name is '%s'", cfg.Casper.TelegramAddress)
		} else {
			res.SetError(fmt.Errorf("cannot get telegram address"), cmds.ErrNormal)
			return
		}
	}
	...
	var extAddrs []string
	if cfg.Swarm.NAT.TraversalSC && len(cfg.Swarm.NAT.StunServers) > 0 {
		log.Infof("NAT traversal is enabled and StunServers found in config. Trying to get external IP...")
		laddr := net.JoinHostPort("0.0.0.0", cfg.Casper.ConnectionPort)
